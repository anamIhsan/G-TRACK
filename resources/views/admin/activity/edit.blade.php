...
@section("content")
    <div class="grid grid-cols-12 gap-4 md:gap-6">
        <form
            x-data="sectionForm()"
            action="{{ route("admin.activities.update", $data->id) }}"
            method="POST"
                        ...
                        <div>
                            ...
                            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                ...
                                >
                                    ...
                                    <option
                                        {{ old("type", $data->type) === "SEKALI" ? "selected" : "" }}
                                        value="SEKALI"
                                    ...
                                    <option
                                        {{ old("type", $data->type) === "TERJADWAL" ? "selected" : "" }}
                                        value="TERJADWAL"
                                        class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                                    >
                                        Terjadwal
                                    ...
                                    <div>
                                        ...
                                        >
                                            <div class="relative">
                                                ...
                                                <div
                                                    :class="switcherToggleBelumMenikah ? 'bg-brand-500 dark:bg-brand-600' :
                                                    'bg-gray-300 dark:bg-white/10'"
                                                    class="block h-6 transition-colors duration-300 ease-in-out rounded-full w-11"
                                                ></div>

                                                <div
                                                    :class="switcherToggleBelumMenikah ? 'translate-x-full' : 'translate-x-0'"
                                    ...
                                    <div>
                                        ...
                                        >
                                            <div class="relative">
                                                ...
                                                <div
                                                    :class="switcherToggleSudahMenikah ? 'bg-brand-500 dark:bg-brand-600' :
                                                    'bg-gray-300 dark:bg-white/10'"
                                                    class="block h-6 transition-colors duration-300 ease-in-out rounded-full w-11"
                                                ></div>

                                                <div
                                                    :class="switcherToggleSudahMenikah ? 'translate-x-full' : 'translate-x-0'"
                                                    class="absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white shadow-theme-sm transition-transform duration-300 ease-linear"
                                                ></div>
                                            </div>
                                ...
                                <div class="flex flex-wrap items-center gap-6 sm:gap-8">
                                    @forelse ($age_categories as $age_category)
                                        <div
                                            x-data="{
                                                switcherToggle:
                                                    {{ in_array($age_category->id, old("age_category_ids", $data->ageCategories->pluck("id")->toArray())) ? "true" : "false" }},
                                            }"
                                        >
                                            <label
                                                for="togel{{ $age_category->id }}"
                                                class="flex items-center gap-3 text-sm font-medium text-gray-700 cursor-pointer select-none dark:text-gray-400"
                                            >
                                                <div class="relative">
                                                    <input
                                                        name="age_category_ids[]"
                                                        value="{{ $age_category->id }}"
                                                        type="checkbox"
                                                        id="togel{{ $age_category->id }}"
                                                        class="sr-only"
                                                    ...
                                                    <div
                                                        :class="switcherToggle ? 'bg-brand-500 dark:bg-brand-600' :
                                                        'bg-gray-300 dark:bg-white/10'"
                                                        class="block h-6 transition-colors duration-300 ease-in-out rounded-full w-11"
                                                    ></div>
        ...
        function sectionForm() {
            return {
                ...
                async getVillages(zone_id, reset = true) {
                    if (reset) {
                        this.village_id = this.village_id;
