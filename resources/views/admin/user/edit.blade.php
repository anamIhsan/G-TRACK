                        ...
                        <div>
                            ...
                            <div x-data="{ showPassword: false }" class="relative">
                                <input
                                    ...
                                    placeholder="Isi jika password ingin diganti"
                                    name="password"
                        ...
                        <div>
                            ...
                            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                ...
                                >
                                    ...
                                    <option
                                        {{ $data->status_kawin === "SUDAH" ? "selected" : "" }}
                                        value="SUDAH"
                                        class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                                    >
                                        Sudah Menikah
                                    ...
                                    <option
                                        {{ $data->status_kawin === "BELUM" ? "selected" : "" }}
                                        value="BELUM"
                                        class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                                    >
                                        Belum Menikah
                            ...
                            <div>
                                ...
                                <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                    <select
                                        ...
                                        :class="isOptionSelected && 'text-gray-800 dark:text-white/90'"
                                        x-model="zone_id"
                                        @change="getVillages(zone_id); fetchInterests(zone_id); getWorks(); fetchMetafields(); fetchCommunities(); isOptionSelected = true"
                                    >
                                        <option
                                            class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                                            disabled
                            ...
                            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                ...
                                >
                                    ...
                                    <template x-for="interest in interests" :key="interest.id">
                                        <option
                                            :value="interest.id"
                                            x-text="interest.nama"
                                            :selected="interest.id == interestId"
                                        ></option>
                                    </template>
                                </select>
                                <span
                    ...
                    <div class="p-5 space-y-6 border-t border-gray-100 sm:p-6 dark:border-gray-800">
                        <div class="flex flex-wrap items-center gap-6 sm:gap-8">
                            <template x-for="community in communities" :key="community.id">
                                <label class="flex items-center gap-3 cursor-pointer">
                                    <div class="relative">
                                        ...
                                        <!-- TRACK -->
                                        <div
                                            :class="selectedCommunityIds.includes(community.id)
                                                ? 'bg-blue-600'
                                                : 'bg-gray-700'"
                                            class="block h-6 transition-colors duration-300 rounded-full w-11"
                                        ></div>
                                        ...
                                        <div
                                            :class="selectedCommunityIds.includes(community.id)
                                                ? 'translate-x-full'
                                                : 'translate-x-0'"
                                            class="absolute top-0.5 left-0.5 h-5 w-5 bg-white rounded-full transition-transform duration-300"
                                        ></div>
                                    </div>
                            ...
                            <template x-if="communities.length === 0">
                                <h3 class="text-sm text-gray-700 dark:text-gray-400">Data Kosong</h3>
                            </template>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function sectionForm() {
            return {
                ...
                village_id: '{{ old("village_id", $data->village_id) }}',
                group_id: '{{ old("group_id", $data->group_id) }}',
                ...
                async fetchCommunities() {
                    ...
                    try {
                        ...
                        this.selectedCommunityIds = this.selectedCommunityIds.filter((id) =>
                            this.communities.some((c) => c.id === id),
                        );
                    } catch {
                        this.communities = [];
                    }
                ...
                async getVillages(zone_id, reset = true) {
                    ...
                    if (reset) {
                        ...
                        this.getGroups(this.village_id);
                    }
                },

                async getGroups(village_id, reset = true) {
                    if (!village_id) return;
                    const res = await fetch(`/api/get-groups/${village_id}`);
                    this.groups = await res.json();
                    if (reset) this.group_id = this.groups[0]?.id || '';
                },
                ...
                async init() {
                    if (this.zone_id) {
                        await this.getGroups(this.village_id, false);
