...
@section('title', 'ADMIN | Edit Kelompok')

@section('content')
    <div class="grid grid-cols-12 gap-4 md:gap-6">
        <form action="{{ route('admin.groups.update', $data->id) }}" method="POST" enctype="multipart/form-data"
            class="col-span-12" x-data="sectionForm()">
            @csrf
            @method('PUT')
                ...
                <div class="w-full bg-white border border-gray-200 rounded-2xl dark:border-gray-800 dark:bg-white/5">
                    <div class="px-5 py-4 sm:px-6 sm:py-5">
                        <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
                            Edit Kelompok pada kelompok:
                            <span class="font-semibold text-brand-600">{{ $data->village->nama }}</span>
                        </h3>
                    </div>
                    ...
                    <div class="p-5 space-y-6 border-t border-gray-100 sm:p-6 dark:border-gray-800">
                        ...
                        <div>
                            ...
                            <input name="admin_nama" type="text" value="{{ old('admin_nama', $data->admin?->nama) }}"
                        ...
                        {{-- No HP Admin Kelompok --}}
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                No HP Admin Kelompok (Diawali 62)
                            </label>
                            <input name="no_tlp" id="no_tlp" type="text"
                                value="{{ old('no_tlp', $data->admin?->no_tlp) }}" placeholder="Cth: 6289812328765"
                                class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                            @error('no_tlp')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input name="email" type="email" value="{{ old('email', $data->admin?->email) }}"
                        ...
                        <div>
                            ...
                            <div x-data="{ showPassword: false }" class="relative">
                                <input :type="showPassword ? 'text' : 'password'"
                                    placeholder="Masukan password jika ingin diganti" name="password"
