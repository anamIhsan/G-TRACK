...
@section("title", "ADMIN | Edit Pekerjaan")

@section("content")
    <div class="grid grid-cols-12 gap-4 md:gap-6">
        <form
            action="{{ route("admin.works.update", $data->id) }}"
            method="POST"
            ...
            <div class="flex">
                <div class="w-full bg-white border border-gray-200 rounded-2xl dark:border-gray-800 dark:bg-white/5">
                    <div class="px-5 py-4 sm:px-6 sm:py-5">
                        <h3 class="text-base font-medium text-gray-800 dark:text-white/90">Ubah Pekerjaan</h3>
                    </div>
                                ...
                                <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                                    ...
                                    >
                                        ...
                                        @foreach ($zones as $zone)
                                            <option
                                                selected="{{ old("zone_id", $data->zone_id) == $zone->id ? "selected" : "" }}"
                                                value="{{ $zone->id }}"
                                                class="text-gray-700 dark:bg-gray-900 dark:text-gray-400"
                                            >
                                                {{ $zone->nama }}
